...
- **Epson LW-PX700 BBIA-1 ferrule batch:** [`bbia1_cn_labels_epson.md`](labels/bbia1_cn_labels_epson.md) and [`bbia1_cn_labels_epson.csv`](labels/bbia1_cn_labels_epson.csv) — 164 label rows (`Wire` / `Location` / `Signal`) for identifying each cut OEM connector end. It intentionally has no new Mesa destination column.
