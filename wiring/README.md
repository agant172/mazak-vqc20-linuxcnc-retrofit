...
- **Excel workbooks archived 2026-09-06.** The consolidated wiring master, the
  Visio crosswalk workbook, and the bom I/O workbook were all legacy 132-row
  snapshots never regenerated for the 124-row authority; they and their sibling
  CSVs now live in [`../archive/workbooks/`](../archive/workbooks/). Current,
  always-regenerated views are the I/O Navigator (`../io-dashboard/`) and the
  label/wire-sheet set under [`labels/`](labels/).
...
- Drawing-book index: [`electrical_diagram_index.md`](electrical_diagram_index.md)
  — drawing-number → PDF-page map for `41434WB.pdf`.
- NC connector inventory: [`nc_connector_inventory.md`](nc_connector_inventory.md)
  and landing audit: [`nc_circuit_landing_audit.md`](nc_circuit_landing_audit.md).
...
- **Troubleshooting quick reference = the BBIA-1 wire reference sheet (owner decision 2026-09-06).** The WireViz / QElectroTech diagram sets were removed that day — scattered over dozens of pages and never liked. A condensed physical-diagram format that fits on a sheet or two is still being looked for; until one exists, the laminated sheet below plus the I/O Navigator are the reference.
...
- **Epson 6 mm Mesa-end ferrule batch:** [`bbia1_mesa_end_ferrules_epson.md`](labels/bbia1_mesa_end_ferrules_epson.md) and [`bbia1_mesa_end_ferrules_epson.csv`](labels/bbia1_mesa_end_ferrules_epson.csv) — every BBIA conductor the pin authority lands on a 7i84U terminal (41 rows as of 2026-09-06, derived directly from `mesa/current_pin_authority.csv` — there is no separate crosswalk to maintain). Only `Label_Text` (for example `B-TB3-07`) is printed; every row prints `HOLD` until its authority row reaches a verified state; the contested CN4-1 row is `HOLD_DISPUTED_PIN` (see authority_conflicts.md §7.1).
