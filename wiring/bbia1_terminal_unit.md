...
- **CN11/CN12 on the SSR board** use the same 20-pin family but with `MR20-RMD` (chassis) / `MR20-LFH` (cable) shells

## Connector map (confirmed from schematic, pg 82)

| Ref | Shell | Pins | Routes to (outside connector) | Function summary |
|-----|-------|-----:|-------------------------------|------------------|
| CN1 | MR-20RMW | 20 | CA3 (MS3102A-20-29P) | Tool clamp / gear / 2nd-S / head lube / Z-axis ZRN dec. / spares |
| CN2 | MR-50RMW | 50 | CA4 (MS3102A-28-21P) | Magazine, tool-measure, door interlock, E-stop, +LY2 2nd over-travel, X/Y ZRN dec. |
| CN3 | MR-50RMW | 50 | TB5 (spindle controller) | E-stop, spindle SET/SRN/SRI, orient, way-lube alarm, ±LY, ±LZ, oil-temp, spindle timer |
| CN4 | MR-20RMW | 20 | FR-SX CON1 (spindle controller) | Spindle zero-speed, SS2/FA/FC, MS/OS, SET1/2, SRN/SRI, orient loop |
...
| CN11-SSR | MR-20 (SSR-bd, MR20-RMD/LFH) | 20 | CB panel loads (SSR outputs) | The **SSR board's own** connector (drawing **4143175309**, sheet 78) — mirrors terminal-unit CN11's 20 functions pin-for-pin (magazine CW/CCW, tool unclamp, gear-shift hi/lo, spindle/work blast, mist coolant, arm extend, air jet, dust inhale, oil hole, flood coolant, magazine cover close, flood-coolant motor starter, hydraulic pump/head-lube pump, 0G), same wire numbers both ends (the earlier "+500/+600 offset" was a 7->2 digit misread - authority_conflicts sec 7.5) |
| CN12 | MR-20RMW (terminal unit) → MR20-RMD/LFH (SSR board) | 9 populated downstream | CB panel loads (SSR outputs, 2nd bank) | 2PC/pallet-related outputs; the downstream SSR pin/wire table is transcribed from 4143175309 p78, but the terminal-unit hop is not independently pin-traced. |
...
1. **BBIA-1 terminal-unit CN11 (MR-20RMW)** — 20 pins, drawing 4143075322 (sheet 85). This is the row labeled `CN11` above and in the CSV. Full pinout: [`bbia1_cn_pinouts.md`](bbia1_cn_pinouts.md).
...
3. **E-stop conductors (EMB / EMC / \*ESP on CN2-40/CN5-4, CN2-41/CN5-5, CN3-1)** stay in the OEM hardwired chain (owner decision 2026-08-15; no new safety relay), never on a Mesa GPIO. LinuxCNC/HAL is not a safety input.
...
Wire numbers and signal names are read directly from the Mazak schematic set (`41434WB.pdf`, drawings 4143075321 / 4143075322 / 4143075323 — Terminal-Unit Details sheets 1–3), OCR-assisted and visually verified page-by-page against the same set used elsewhere in this repo. Cross-referenced against the Mazak Corporation project files delivered 2025-10-13.
