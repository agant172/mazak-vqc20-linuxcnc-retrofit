...
213 photos (8 triage batches, `findings_00.md`–`findings_07.md`) of the Mazak VQC 20/40 (SN 060231, Mazatrol CAM M-2, ladder YM2V39L) were reviewed for the LinuxCNC/Mesa 7i80HDT + 7i44 + 7i49 + two-7i84U retrofit; P2 is unused (confirmed 2026-08-13 by `readhmid`, flipped from the P3-unused assumption in place when this survey was written). Tally across the eight batch files: **134 HIGH**, **46 MEDIUM**, **34 LOW** (one batch's count is off by one due to a summary-line artifact — treat as ~213/134/45/34).
...
| AC spindle motor | Mitsubishi AC SPINDLE MOTOR, TYPE **SE-EV-FV** (suffix CONFIRMED 2026-08-12) | 4-pole; **CONT** 3.7/3.7/2.2 kW and **1/2H** 5.5/5.5/3.7 kW at **RPM 1500/4500/6000**; V ~~150~~ **130**/170/170 (both duty columns); A ~~26~~ **28**/20/13 and 39/29/21; SERIAL D91400020; part no. 995196-03; Klixon 9700L-246-215 (open 150°C / close 99°C, **24VDC 12A**); blower IA-15040 **1-phase** 200V 0.2A | IMG_2065 + batch 3 (2026-08-12) | **CORRECTED** — a sharp nameplate close-up superseded two IMG_2065 misreads (V 150→130, A 26→28) and resolved the TYPE suffix. The six kW/V/A columns are two duty ratings (CONT, 1/2H), not six taps. See [`spindle_motor_plg_encoder.md`](spindle_motor_plg_encoder.md#corrections-to-photo_survey_miscmd) |
...
- ~~**AC spindle motor TYPE suffix** ("SE-EV-FV", superscript character unclear) and **serial** ("D9140002_0", last digit unclear) — `IMG_2065`, single read, not cross-confirmed by any other photo.~~ **RESOLVED 2026-08-12** by a sharp nameplate close-up (batch 3): TYPE is **`SE-EV-FV`**, serial **`D91400020`**. The same read also corrected two `IMG_2065` values — V 150→**130**, A 26→**28** — so treat other single-read figures from that frame with matching caution. Date field is worn beyond reading in both sets. See [`spindle_motor_plg_encoder.md`](spindle_motor_plg_encoder.md#corrections-to-photo_survey_miscmd).
