...
| S10 | Mesa 7i80HDT product page (100BaseT, 72 I/O, three 50-pin connectors, 5V-tolerant) | <https://store.mesanet.com/index.php?product_id=386&route=product/product> |
...
| 3 | 7i80HDT has 100BaseT Ethernet, three 50-pin daughter connectors (P1/P2/P3), 72 I/O, 5V-tolerant | Verified | S10, the Mesa 7i80HDT product page, states all four properties directly. S4/S5/S9 are historical 7i80HD sources and are not needed to infer HDT capability. | Keep the capability claim. Distinguish base GPIO from daughter-card overlays: P1 is physically routed through the 7i44 RS-422 transceivers, P2 through the 7i49 resolver/analog interface, and only empty P3 remains accessible as raw connector GPIO. |
...
| 14 | `7i80hdt_7i44_ss_7i49d.bit` is the correct bitfile | Unverified | S7/S8 (PCW supplied 7i80HDT bitfiles for 7i44+7i48 and for svss6_8d, not for 7i44+7i84×2+7i49); repo lacks an actual binary, SHA-256, source/build provenance, IDROM dump, or pin listing | Add `UNVERIFIED` banner to every place the bitfile name appears in active spec files. Deliverable D3 in `pre_power_deliverables.md` covers bitfile provenance (SHA-256, source/build provenance, IDROM readback, and pin dump). |
| 15 | On P3, `gpio.042` is the probe input | Contradicted (stale) | `mesa/README.md` "The probe was moved from a former P3 `gpio.042` binding to 7i84U-B input-15" — the migration is already the current design | RETRACTED. Active-voice references cleaned up in commit `1a11c14` (2026-08-07): `linuxcnc/motion_7i80hdt.hal`, `linuxcnc/mazak_vqc_20_40.hal`, `docs/architecture_decision.md`, `wiring/io_map_research_notes.md`, `docs/project_status.md`, and `archive/crosswalk/element_dashboard_crosswalk_summary.md` now all state the current design: P3 unused/spare, probe on 7i84U-B TB3 IN15 (opto-isolated 24 V). Bare 3.3 V FPGA GPIO must not carry 24 V field wiring. |
...
| 18 | Resolver is a device separate from the drive's tachogenerator | Unverified from a primary electrical/manual source | Machine-photo interpretation and repo reasoning place the TS2014N at the ball-screw tail and the TGF-3D in the motor/TRA velocity loop, but a repo-internal explanation is not primary-source verification. | Keep only as the working machine topology and mark it pending the exact Mitsubishi/Mazak schematic or a traced as-built cable record. Do not classify it verified until that evidence is committed. |
...
| 20 | 7i84U outputs can directly drive 100 VAC solenoids | Contradicted | The Mesa 7i84U manual specifies 5-28 VDC field power/output operation; `linuxcnc/field_7i84u.hal` and `docs/pre_power_deliverables.md` require interposing relays for AC loads. | Repo forbids direct 100 VAC drive. Any archival wording that hinted at direct AC drive is retracted. |
...
| 22 | `index-divisor=1` yields exactly one HOME index per mechanical revolution | Unverified | Value is a hostmot2 resolver parameter — the correct setting depends on how many electrical cycles the resolver produces per mechanical revolution, which for TS2014N size-25 is claimed to be 1 but has not been counted in-repo | Downgrade to `MEASURED_PENDING`. Verify by rotating a de-energized ball screw one turn and counting index pulses on `resolver.00.index-enable`. **2026-08-17 update: `index-divisor = 1` is now positively wrong and the INI carries `5`.** Each of the X, Y and Z ballscrews was hand-turned one revolution and moved **10 mm** (measured, machine unpowered); with the confirmed 1:1 resolver coupling and the 2.000 mm grid spacing from τ = 2, that is 5 electrical cycles per screw revolution. Still `MEASURED_PENDING` — the pulse count itself has not been taken, and the 2.000 mm is derived. See `docs/ballscrew_lead_2026-08-17.md`. |
| 23 | `RESOLVER_SCALE = 1.0` is a safe commissioned resolver value | Unverified and unsafe to assume | HostMot2 defines resolver `.scale` as **machine units per resolver electrical revolution**, not counts per user unit. The correct signed value depends on ball-screw lead and resolver-to-screw ratio. | Keep 1.0 only as a conspicuous placeholder while the fail-off drive hold remains FALSE. Measure machine travel against `rawcounts` over multiple revolutions, then replace it with signed machine-units-per-electrical-revolution before any motion. Do not set scale to zero as a safety mechanism; use the explicit output hold and physical drive isolation. |
...
- `linuxcnc/motion_7i80hdt.hal` and `mazak_vqc_20_40.hal` — resolver scale is documented in the correct units and held as an uncommissioned placeholder; the active HostMot2 config uses `num_encoders=0` until the spindle feedback hardware and pins are assigned.
