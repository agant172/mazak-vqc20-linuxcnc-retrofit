...
| 22 | `index-divisor=1` yields exactly one HOME index per mechanical revolution | Unverified | Value is a hostmot2 resolver parameter — the correct setting depends on how many electrical cycles the resolver produces per mechanical revolution, which for TS2014N size-25 is claimed to be 1 but has not been counted in-repo | Downgrade to `MEASURED_PENDING`. Verify by rotating a de-energized ball screw one turn and counting index pulses on `resolver.00.index-enable`. **2026-08-17 update: `index-divisor = 1` is now positively wrong and the INI carries `5`.** The Z ballscrew was hand-turned one revolution and moved **10 mm** (measured, machine unpowered); with the confirmed 1:1 resolver coupling and the 2.000 mm grid spacing from τ = 2, that is 5 electrical cycles per screw revolution. Still `MEASURED_PENDING` — the pulse count itself has not been taken, and the 2.000 mm is derived. See `docs/ballscrew_lead_2026-08-17.md`. |
