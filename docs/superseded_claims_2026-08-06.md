...
| 15 | On P3, `gpio.042` is the probe input | Contradicted (stale) | `mesa/README.md` "The probe was moved from a former P3 `gpio.042` binding to 7i84U-B input-15" — the migration is already the current design | Every remaining `gpio.042 = probe` reference in active docs is stale. Retract in a follow-up pass; the electrical authority (`current_pin_authority.csv`) already routes PROBE to `IN15` on 7i84U-B. |
