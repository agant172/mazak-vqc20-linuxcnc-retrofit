...
- 385-signal ladder crosswalk; 124 current authority rows covered in io-dashboard.
...
- [x] **2026-09-05 (at the machine): TB2 inputs IN16-20 confirmed live** (`tool-unclamped`, `gear-hi-conf`, `gear-lo-conf`, `mag-bcd-bit0`, `mag-bcd-bit1`) — same 24V/TB1-8 return pair, each correctly landed on the first real attempt. One gotcha along the way: TB3 and TB2 are two adjacent 24-pin blocks with no gap/label at the boundary, so "last pin of TB3" (TB3-16, `INPUT15`) reads identically to "start of TB2" (TB2-1, `INPUT16`) by feel — cost some back-and-forth until the HAL watch was widened across both sides of the boundary. **End of session: the single test jumper ended up resting on TB3-16 (IN15), not back on TB3-14 — the IN13 bench jumper for item 46 is NOT currently active**, move it back to TB3-14 if that interim state is still wanted. Full writeup: [`commissioning_logs/sserial_enumeration_check_2026-09-05.md`](commissioning_logs/sserial_enumeration_check_2026-09-05.md#update-2026-09-05-later-still--tb2-inputs-confirmed-and-a-second-gotcha).
...
- [x] ~~**Then buzz `CN3-39` and `CN3-44` at the BBIA-1 board**~~ — **dissolved 2026-09-02** as a citation defect (`authority_conflicts.md` § 7.5): CN3-44 = SPTD spindle tool detector, CN3-39 = 147 tool detector; PRS-55/66 are at CN2-14 / CN1-5. The PRS-55/66 "look on the machine" item above still stands. *Original text:* — two OEM sources disagree about what is on them ([`../wiring/authority_conflicts.md`](../wiring/authority_conflicts.md) §7.2). Dwg 4143075409 pg135 calls them the 2nd −Z / 2nd +Y limits (`PRS-66` / `PRS-55`); the terminal-unit pinout calls them `147` OIL TEMP DETECTOR and `SPTD` SPINDLE TIMER. Note only 24 of CN3's 50 pins are transcribed and the CND→CN index is **not** preserved across the pass-through, so both sources may be right about *different* pins. ⚠️ **Neither conductor may be landed on a Mesa input until this is settled.**
...
- [x] ~~**Buzz `CN2-14`**~~ — **settled 2026-09-02** (§ 7.5): CN2-14 is `+LY2`, the second +Y over-travel (PRS-55); `Z_LIMIT_PLUS` is genuinely unlocated and the B-TB3-05 ferrule was withdrawn. *Original text:* for continuity to the +Z over-travel limit switch *and* to `CN6-12` — three sources disagree about whether it is `+LTZ` Z-axis over travel, unlabelled, or a **combined +Y/+Z bus** (`+LYZ`) ([`../wiring/authority_conflicts.md`](../wiring/authority_conflicts.md) §7.3). Continuity to both +Y and +Z confirms the combined-bus reading. ⚠️ This pin is already on a printed ferrule (`B-TB3-05`), now released as `HOLD_DISPUTED_PIN` — do not land it until this resolves.
...
- [x] 1. **Done 2026-09-05** (bench): `ping 10.10.10.121`, `hm2_eth` load and `readhmid` all good — `docs/commissioning_logs/sserial_enumeration_check_2026-09-05.md`. Original: Confirm the host/network path first, since everything below depends on it: host NIC `enp0s31f6` (**name unverified** — check `ip -o link show`) at 10.10.10.1/24, board static 10.10.10.121, `ping 10.10.10.121`, and `hm2_eth` `board_ip="10.10.10.121"`.
- [ ] 2. *(7i44 proven seated — 7i84U-A enumerates through it, 2026-09-05. 7i49 presence still unproven: resolver pins instantiate from firmware regardless; needs the excitation scope check.)* Seat **7i49 on P1** and **7i44 on P3** with the 50-pin IDC cables. **P2 stays empty — no exceptions**: it is bare 3.3 V FPGA GPIO and must never see 24 V field wiring.
- [ ] 3. *(A done 2026-09-05 — enumerates on channel 0 with VIN + VFIELDA/B powered; TB3/TB2 inputs proven live. B pending — see owner question below on whether it is on hand.)* Bring up **7i84U-A on 7i44 channel 0** and **7i84U-B on channel 1**, both under HostMot2 smart-serial port 0. Preserve the A/B identities — do not let them swap silently.
- [x] 4. **Done 2026-09-05** — byte-identical to the 2026-08-13 baseline (`--readhmid` reports the FPGA config, not daughter-card presence). Original: Re-run `mesaflash --readhmid` **after** the cards are seated and diff it against [`../mesa/firmware/readhmid_2026-08-13.txt`](../mesa/firmware/readhmid_2026-08-13.txt) (bench read, host only). Confirm the daughter cards now enumerate.
- [ ] 5. *(A appears, 2026-09-05; B does not — expected while B is not wired.)* Confirm both remotes appear: `halcmd show pin hm2` shows `hm2_7i80.0.7i84.0.0.*` (A) and `hm2_7i80.0.7i84.0.1.*` (B). If only one appears, stop — do not renumber to make it fit.
...
> [`../background/handoff.md`](../background/handoff.md) (moved to the shelf 2026-09-06). **This file is the authority for the task
...
### Repo hygiene — from the 2026-09-06 relevance audit

Full findings: [`repo_relevance_audit_2026-09-06.md`](repo_relevance_audit_2026-09-06.md).
Moves and 22 stale-statement fixes were made that day; these are the decisions left.

- [x] **(2026-09-06) `diagrams/` and both generators REMOVED — owner decision 2026-09-06.** "They look terrible and are scattered over dozens of pages." WireViz and QElectroTech sets (125 files) plus `generate_wireviz_diagrams.py` / `generate_qelectrotech_project.py` deleted outright (git history keeps them). **The BBIA-1 wire reference sheet (`wiring/labels/bbia1_wire_reference_sheet.html`) is the troubleshooting quick reference for now**; a cleaner, condensed physical-diagram format is still being explored — no tool chosen. `generate_interface_crosswalks.py` and `wiring/plane_a_bbia1_pin_crosswalk.csv` stay (the 320-row Plane A accounting is independent of the drawings).
- [x] **(2026-09-06) `docs/manual_set/` pulled from the tree and git-ignored — owner decision 2026-09-06.** The committed Rev B PDFs were built 2026-08-07 from 132 rows, before the 2026-08-23 builder fixes for retracted P1/P2/P3 claims, and never rebuilt. `scripts/build_manual_set.py` and its CI smoke-build stay; build locally when a printed set is wanted. Still open on the builder: `build_manual_set.py:995-1000` points at maintenance-record paths that do not exist — fix before the next real build.
- [ ] **(2026-09-06) Put `generate_interface_crosswalks.py` in the CI gate** — `wiring/plane_a_bbia1_pin_crosswalk.csv` is 7 notes rows behind the authority. Closure: `git diff` clean after the gate regenerates it.
- [ ] **(2026-09-06) Add the seven hand-installed health files to `install_health.sh`** — `backup_watch.py`, `notify_failure.sh`, `repo_guard.sh`, `mazak-backup-watch.{service,timer}`, `mazak-repo-guard.{service,timer}`, `mazak-notify-failure@.service` are live on the OptiPlex but no installer deploys them. Closure: fresh install from the script reproduces `systemctl list-timers`.
- [ ] **(2026-09-06) Relocate `scripts/photo_tools/`** (general camera-roll tool, not retrofit) to `claude-config` or `project-docs`; then drop `.gitignore:58-65`. Owner call.
- [x] **(2026-09-06) 7i84U-B is ON ORDER — owner confirmed 2026-09-06.** Only 7i84U-A is on the bench. The "all hardware on hand 2026-08-17" line in the Closed section was wrong and is annotated.
- [ ] **(2026-09-06) Align the photo count** across `README_photo_sorting.md` (723), `photo_reindex_2026-08-21.md` (936) and `CLAUDE.md` (1142 objects, rclone 2026-08-23 — the one to keep).
- [ ] **(2026-09-06) Fix the 7i80HDT jumper roles** in `docs/cabinet_photo_checklist.md:34` and `mesa/mesa_firmware_checklist.md:211` — manual: W2/W3 = IP, W1 = flash select, W4 = 5 V tolerance, W5–W7 = daughtercard power. Closure: both lines cite `7i80hdtman.pdf` page.
- [ ] **(2026-09-06) Small stale lines** — `docs/servo_amp_analysis.md:23` (Z motor is `HD101-12-TT-A` per the parts list), `docs/photo_survey_misc.md:6` (`Misc. Photos` never existed — add banner), `docs/crt_screen_survey_2026-08-21.md:38`, `linuxcnc/atc_orient.ini.snippet` (`ORIENT_TIMEOUT` 15 vs INI 45), `io-dashboard/README.md` C1–C10 register (only C3/C6/C9/C10 exist), `io-dashboard/project_data.js` frozen at 2026-08-11.
- [ ] **(2026-09-06) `scripts/host_status/retired/` (8 files)** — nothing runs it; move under `archive/` with two link edits, or leave. Owner call.
...
- [x] **All Mesa interface hardware on hand (2026-08-17).** *(**Correction 2026-09-06, owner:** 7i84U-B was **not** on hand — it is on order. Everything else in this line stands.)* 7i80HDT, 7i49, 7i44, 7i84U-A, 7i84U-B and the 50-pin IDC cables. On-hand is inventory, not evidence: only the 7i80HDT has been proven present electrically, so every daughter-card pin name stays a placeholder until Phase B step 6.
...
1. Confirm 7i80HDT detection over Ethernet (host static IP 10.10.10.1/24 on the Mesa NIC, `ping 10.10.10.121`, `mesaflash --device ETHER --addr 10.10.10.121 --readhmid`, `hm2_eth` HAL load). *(Addresses updated 2026-09-06 for the 2026-08-23 renumber; `--device ETHER` is the form that works with mesaflash 3.5.x.)*
