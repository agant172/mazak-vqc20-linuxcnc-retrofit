...
- [ ] **(2026-09-06) Decide the fate of `diagrams/` (125 files, all stale, no CI).** Both sets draw ORC1 (CN3-14 → OUT4) as a HOLD released 2026-09-03. Recommendation: keep WireViz only, drop the 62-file QElectroTech set, and either add `generate_interface_crosswalks.py` + `generate_wireviz_diagrams.py` to the authority gate or stop committing rendered output. Closure: diagrams regenerate clean in CI, or the folder is gone.
...
- [ ] **(2026-09-06) Owner: is 7i84U-B on hand or on order?** `project_status` "Closed" says on hand 2026-08-17; the 2026-09-05 bench record says on order. Fix whichever is wrong.
