...
- [ ] ⚠️ **Renumber the Mesa control subnet off `192.168.1.0/24` — owner decision 2026-08-23, target `10.10.10.0/24`.** The workshop LAN is *also* `192.168.1.0/24` (iMac `.19`, gateway `.1`) and this PC holds **`192.168.1.1`** on the Mesa NIC — the gateway's own address. Two consequences: patching the Mesa NIC into the shop switch would make this PC fight the router for `.1` and take down the shop network; and the route `192.168.1.0/24 dev enp0s31f6` already makes every shop-LAN address unreachable from this box, today, by sending it out the Mesa link into a dead end. **Board EEPROM already written to `10.10.10.121` (2026-08-23) — inert until jumper W3 moves.** Remaining: move **W3 UP** on the 7i80HDT (W2 stays DOWN; `docs/Mesa Manuals/7i80hdtman.pdf`, IP ADDRESS SELECTION), power-cycle, run `scripts/health/switch_mesa_subnet.sh`, then update `board_ip` in `linuxcnc/mazak_vqc_20_40.hal:55` and the ~7 docs citing the old addresses. **Rollback:** W3 back DOWN → board answers at `192.168.1.121` again; it cannot be bricked. **Closure test:** `ping 10.10.10.121` plus a clean `readhmid`, and `ping 192.168.1.19` reaching the iMac.
