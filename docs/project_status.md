...
- [ ] Execute the hm2_eth NIC pinning and multi-hour latency-under-load acceptance per [`hm2_eth_nic_validation.md`](hm2_eth_nic_validation.md); record NIC part / MAC / driver / IRQ affinity and wire `hm2_7i80.0.packet-error-exceeded` and `hm2_7i80.0.watchdog.has_bit` into the motion-permit chain before enabling drives.
- [ ] Build the resolver + analog cables per [`grounding_shielding_plan.md`](grounding_shielding_plan.md) (three individually shielded twisted pairs per resolver, shields at 7i49 end only; segregation Class A/B ≥ 150 mm from Class D). Execute the noise-survey acceptance (static baseline, contactor-firing, spindle-only, axis-motion) and log results under `docs/commissioning_logs/`.
- [ ] Add `hm2dpll` to config and route sserial + resolver reads through a DPLL slot; verify smart-serial latency budget end-to-end (unplug hm2_eth during jog, confirm cascaded watchdog trips). See [`smart_serial_latency.md`](smart_serial_latency.md).
