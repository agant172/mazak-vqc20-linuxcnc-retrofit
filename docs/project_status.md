...
- [x] **HMI screen chosen — QtDragon HD, owner decision 2026-09-02.** `[DISPLAY]` in [`../linuxcnc/mazak_vqc_20_40.ini`](../linuxcnc/mazak_vqc_20_40.ini) is `DISPLAY = qtvcp qtdragon_hd` (INI v1.2), replacing Axis. This closes the variant question, so ATC / hydraulic / orient tab work can be laid out against the HD geometry. **The decision commits a 1920x1080 LANDSCAPE panel** — `qtdragon_hd` is drawn for that geometry, and the alternatives (`qtdragon` small, `qtdragon_hd_vert` portrait) lay custom tabs out differently. **Still open:** the physical cabinet panel is not bought or fitted; it must be landscape and at least 1920x1080 or the screen is cropped or scaled. Developed on the bench monitor (HDMI-1, 3440x1440), where the screen opens its own 1920x1080 window.
- [x] **`python3-poppler-qt5` installed — 2026-09-02, at the machine.** `21.3.0-3+b3`; `import popplerqt5` succeeds. Without it QtDragon logs `PDFViwer - Is python3-poppler-qt5 installed?` at every start and the setup tab's PDF pane is dead. **Scope note:** the stock binding is a *per-program setup sheet* — `qtdragon_hd_handler.py` looks for `<loaded-program>.pdf` beside the G-code file (line ~1160) and otherwise shows a built-in sample via `PDFViewer.loadSample()`. It is **not** a manual browser. Pointing it at the OEM manual set for SN 060231 needs a handler override calling `PDFViewer.loadView(path)`, which is custom work, not configuration.
...
- [ ] **(2026-09-02) Resolve the 4000-vs-6000 RPM spindle top-speed conflict BEFORE the spindle turns.** `linuxcnc/mazak_vqc_20_40.ini` `[SPINDLE_0]` sets `MAX_FORWARD_VELOCITY = 4000` and `OUTPUT_SCALE = 4000`, justified only by a comment asserting "nameplate max on this class of machine is 4000 RPM" — no nameplate is cited. Two physical labels say otherwise: the SX-CPU2 board sticker reads `BASE 1500 RPM / TOP 6000 RPM` ([`../background/frsx_orient_detector_capture.md`](../background/frsx_orient_detector_capture.md) line 58) and the spindle motor's photographed nameplate carries a ratings table columned @1500/@4500/@6000 rpm ([`spindle_motor_plg_encoder.md`](spindle_motor_plg_encoder.md) line 194). BBIA-1 CN4-20 (SE3) is labelled "SPEED REFERENCE 3 (10V MAX SPEED)", so 10 V = top speed: if top speed is really 6000, `OUTPUT_SCALE = 4000` makes every commanded speed run **50% fast**. Closure test: at commissioning step 12, command a known low RPM with drives inhibited, meter the AOUT3 voltage, and tach the spindle at first rotation. Correct `[SPINDLE_0]` and `[DISPLAY] MAX_SPINDLE_0_SPEED` together. Found while drafting the QtDragon `[DISPLAY]` block (INI v1.2).
