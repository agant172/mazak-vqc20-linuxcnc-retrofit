...
- Firmware `7i80hdt_rmsvss6_8.bin` is flashed and its layout/identity confirmed by two independent `readhmid` reads (2026-08-11 flash-time, 2026-08-13 re-check, byte-identical) plus a recorded SHA-256; the upstream Mesa/PCW source citation for the binary itself is still open — see [`../mesa/mesa_firmware_checklist.md`](../mesa/mesa_firmware_checklist.md#bitfile-provenance-verification-procedure).
